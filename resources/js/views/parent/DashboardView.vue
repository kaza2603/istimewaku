...
onMounted(async () => {
    ...
    if (auth.currentUser?.email) {
        try {
            console.log(
                "Fetching students for parent email:",
...
const fetchMissingData = async () => {
    ...
    for (const child of myChildren.value) {
        ...
        if (child.schoolId && !child.school) {
            console.log(
                `Fetching school data for child: ${child.name} (ID: ${child.schoolId})`,
            );
            try {
                const school = await studentsStore.fetchSchoolById(
                    child.schoolId,
                );
                if (school) {
                ...
                console.error(
                    `Error fetching school for ${child.name}:`,
                    error,
                );
            }
        }
        ...
        if (child.schoolId && child.classId && !child.class) {
            console.log(
                `Fetching class data for child: ${child.name} (ID: ${child.classId})`,
            );
            try {
                const classData = await studentsStore.fetchClassById(
                    child.schoolId,
                    child.classId,
                );
                if (classData) {
    ...
    <div class="p-4 sm:p-6 md:p-8">
        ...
        <Card class="mb-8">
            <CardHeader>
                <CardTitle
                    >Selamat Datang,
                    {{ auth.currentUser?.name || "Ibu Bapa" }}</CardTitle
                >
                <CardDescription>
                    Panel ibu bapa membolehkan anda memantau prestasi anak-anak
                    anda dalam pembelajaran.
                </CardDescription>
            </CardHeader>
            <CardContent>
                <p class="mb-4">
                    Dari sini, anda boleh mendaftar pelajar baharu, memantau
                    prestasi mereka, dan mengakses maklumat pembelajaran
                    terkini.
                </p>
                <div class="flex items-center gap-4">
                    <Button
                        variant="outline"
                        @click="navigateToMyChildren"
        ...
        <Card v-else-if="studentsStore.error" class="mb-8">
            <CardContent class="p-6 text-center">
                ...
                <Button
                    @click="
                        studentsStore.fetchStudentsByParent(
                            auth.currentUser?.email,
                        )
                    "
                >
        ...
        <Card v-else-if="myChildren.length === 0" class="mb-8">
            <CardContent class="p-8 text-center">
                ...
                <h3 class="text-lg font-medium mb-2">
                    Tiada Pelajar Berdaftar
                </h3>
                <p class="text-gray-500 mb-6 max-w-md mx-auto">
                    Anda belum mendaftarkan sebarang pelajar. Daftar pelajar
                    baharu untuk mula pantau prestasi pembelajaran mereka.
                </p>
