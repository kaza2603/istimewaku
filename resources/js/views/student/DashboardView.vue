...
const student = computed(() => {
    ...
    if (studentProfile.value) {
        return {
            name:
                studentProfile.value.name ||
                studentProfile.value.student_name ||
                "-",
            class:
                studentProfile.value.class ||
                studentProfile.value.class_name ||
            ...
            avatarUrl:
                studentProfile.value.avatarUrl ||
                "/images/avatar-placeholder.png",
            email:
                studentProfile.value.email ||
                studentProfile.value.student_email ||
                "",
            username:
                studentProfile.value.username ||
                studentProfile.value.student_username ||
                "",
        };
    }
    ...
    if (auth.currentStudent) {
        return {
            name:
                auth.currentStudent.name ||
                auth.currentStudent.student_name ||
                "-",
            class:
                auth.currentStudent.class ||
                auth.currentStudent.class_name ||
                "Kelas Tidak Diketahui",
            school:
                auth.currentStudent.school ||
                auth.currentStudent.school_name ||
                "Sekolah Tidak Diketahui",
            avatarUrl:
                auth.currentStudent.avatarUrl ||
                "/images/avatar-placeholder.png",
            email:
                auth.currentStudent.email ||
                auth.currentStudent.student_email ||
                "",
            username:
                auth.currentStudent.username ||
...
const studentId = computed(() => {
    ...
    if (auth.currentStudent?.student_username)
        return auth.currentStudent.student_username;
    return null;
});
...
const performanceStats = computed(() =>
    studentPerformanceStore.getPerformanceStats(),
);
const topicPerformance = computed(() =>
    studentPerformanceStore.getTopicPerformance(),
);
const learningAnalytics = computed(() =>
    studentPerformanceStore.getLearningStylePreference(),
);
const weeklyActivity = computed(() =>
    studentPerformanceStore.getWeeklyActivityData(),
);
...
const upcomingLessons = computed(() => {
    ...
    const incomplete = topicPerformance.value
...
const dashboardStats = computed(() => {
    ...
    if (totalMinutes === 0 && weeklyData && Array.isArray(weeklyData)) {
        totalMinutes = weeklyData.reduce(
            (total, day) => total + (day.timeSpent || 0),
            0,
        );
    }
...
const learningStyle = computed(() => {
    ...
    activities.forEach((activity, index) => {
        const quizType =
            activity.quiz_type || quizTypes[index % quizTypes.length];
    ...
    Object.keys(styleStats).forEach((style) => {
        averageScores[style] =
            styleStats[style].count > 0
                ? styleStats[style].totalScore / styleStats[style].count
                : 0;
    });

    // Calculate total of all averages
    const totalAverage = Object.values(averageScores).reduce(
        (sum, avg) => sum + avg,
        0,
    );
...
const fetchStudentProfile = async () => {
    ...
    try {
        ...
        if (auth.currentStudent.user_id) {
            profile = await studentsStore.fetchStudentProfile(
                auth.currentStudent.user_id,
            );
        }
        ...
        if (profile) {
            ...
            console.warn(
                "Could not fetch student profile, using auth store data",
            );
        }
    } catch (error) {
        ...
        toast.error("Ralat", {
            description:
                "Gagal memuat profil pelajar. Menggunakan data tempatan.",
        });
    }
};
...
onMounted(async () => {
    ...
    try {
        ...
        if (studentId.value) {
            ...
            await studentPerformanceStore.fetchStudentAnalytics(
                studentId.value,
            );
            ...
            try {
                allActivitiesData.value =
                    await studentPerformanceStore.fetchAllActivities(
                        studentId.value,
                    );
                console.log(
                    "Loaded all activities:",
                    allActivitiesData.value.length,
                );
            } catch (error) {
                console.warn(
                    "Could not fetch all activities, using recent activities:",
                    error,
                );
            ...
            toast.warning("Amaran", {
                description:
                    "Tidak dapat memuat data prestasi. ID pelajar tidak dijumpai.",
            });
        }
    } catch (error) {
...
<template>
    <div
        class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-yellow-50"
    >
        <!-- Enhanced Header with gradient and better styling -->
        <div
            class="bg-gradient-to-r from-yellow-700 via-yellow-400 to-orange-400 shadow-lg"
        >
            <div class="container mx-auto px-6 py-8">
                <div class="flex justify-between items-center">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h1
                                class="text-3xl font-bold text-white drop-shadow-sm"
                            >
                                Portal Pelajar IstimewaKu
                            </h1>
                            <p class="text-yellow-100 text-sm">
                                Pembelajaran Interaktif & Menyeronokkan
                            </p>
                        </div>
                    </div>
                    <Button
        ...
        <div class="container mx-auto px-6 py-8 -mt-4">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <Card
                        class="bg-white/70 backdrop-blur-sm shadow-xl border-0 overflow-hidden"
                    >
                        <div
                            class="bg-gradient-to-r from-blue-500 to-purple-600 h-20"
                        ></div>
                        <CardContent class="relative pt-0 pb-6">
                            <div class="flex flex-col items-center -mt-12">
                                ...
                                <div class="text-center mt-4">
                                    ...
                                    <div
                                        class="flex items-center justify-center mt-1 text-blue-600"
                                    >
                    ...
                    <!-- Enhanced Quick Menu -->
                    <Card
                        class="bg-white/70 backdrop-blur-sm shadow-xl border-0"
                    >
                ...
                <div class="lg:col-span-3 space-y-8">
                    ...
                    <div
                        class="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border-0 p-6"
                    >
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-gray-600 mt-2">
                                    Mari kita teruskan pembelajaran hari ini
                                </p>
                            </div>
                            <div class="hidden md:block">
                                <div
                                    class="bg-gradient-to-r from-yellow-400 to-orange-500 rounded-full p-4"
                                >
                        ...
                        >
                            <CardContent class="p-6">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p
                                            class="text-blue-100 font-medium mb-1"
                                        >
                                            Kuiz Dicuba
                                        </p>
                                    ...
                                    <div
                                        class="bg-white/20 backdrop-blur-sm rounded-full p-4"
                                    >
                        ...
                        >
                            <CardContent class="p-6">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p
                                            class="text-green-100 font-medium mb-1"
                                        >
                                            Purata Skor
                                        </p>
                                        <p class="text-3xl font-bold">
                        ...
                        >
                            <CardContent class="p-6">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p
                                            class="text-purple-100 font-medium mb-1"
                                        >
                                            Masa Pembelajaran
                                        </p>
                                        <p class="text-3xl font-bold">
                                            {{ dashboardStats.totalTimeSpent }}
                                        </p>
                        ...
                        <CardHeader class="border-b border-gray-100">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <CardDescription
                                        >Aktiviti pembelajaran yang telah anda
                                        selesaikan</CardDescription
                                    >
                                </div>
                    ...
                    >
                        <CardHeader class="border-b border-gray-100">
                            <CardTitle class="flex items-center text-gray-800">
                                <BarChart3
                                    class="mr-3 h-6 w-6 text-purple-500"
                                />
                                Analisis Pembelajaran
                            </CardTitle>
                            <CardDescription
                                >Gaya pembelajaran dan kekuatan
                        ...
                        <CardContent class="p-6">
                            ...
                            <div v-else class="space-y-8">
                                ...
                                <div>
                                    <h4
                                        class="font-semibold text-lg mb-4 text-gray-800"
                                    >
                                        Gaya Pembelajaran
                                    </h4>
                                    <div class="space-y-4">
                                        ...
                                        >
                                            ...
                                            <div
                                                class="w-full bg-blue-200 rounded-full h-3"
                                            >
                                                <div
                                        ...
                                        <div
                                            class="bg-gradient-to-r from-green-50 to-green-100 rounded-xl p-4"
                                        >
                                            <div
                                                class="flex items-center justify-between mb-2"
                                            >
                                            ...
                                            <div
                                                class="w-full bg-green-200 rounded-full h-3"
                                            >
                                                <div
                                        ...
                                        <div
                                            class="bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl p-4"
                                        >
                                            <div
                                                class="flex items-center justify-between mb-2"
                                            >
                                            ...
                                            <div
                                                class="w-full bg-purple-200 rounded-full h-3"
                                            >
                                                <div
                                ...
                                >
                                    ...
                                    >
                                        <h4
                                            class="font-semibold text-lg mb-4 text-green-800 flex items-center"
                                        >
                                        ...
                                        <ul class="space-y-3">
                                            ...
                                            >
                                                <div
                                                    class="bg-green-500 rounded-full w-3 h-3 mr-3"
                                                ></div>
                                                <span
                                    ...
                                    >
                                        <h4
                                            class="font-semibold text-lg mb-4 text-yellow-800 flex items-center"
                                        >
                                        ...
                                        <ul class="space-y-3">
                                            ...
                                            >
                                                <div
                                                    class="bg-yellow-500 rounded-full w-3 h-3 mr-3"
                                                ></div>
                                                <span
                                                    class="font-medium text-gray-700"
                                                    >{{ area.name }}</span
